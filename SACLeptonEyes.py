...
from SACEyesFinder import EyesFinder
...
thSensorWidth = 80
thSensorHeight = 60
EyesSizeUpperLimit = 600
EyesSizeLowerLimit = 300
...
def checkEyesSize(image, currWidth, minWidth, maxWidth):
...
try:
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        # run state machine
        if state == "IDLE":
            if ff.getTcEyesContours(image) == True:
                state = "WAIT_FOR_SIZE_OK"
            else:
                state = "IDLE"
                #print("Eyes not present")
                addText(image, 'Geen ogen gevonden.')


        elif state == "WAIT_FOR_SIZE_OK":
            if ff.getTcEyesContours(image) == True:
                if checkEyesSize(image, ff.getTcEyesROIWidth(), EyesSizeLowerLimit, EyesSizeUpperLimit) == False:
                    state = "WAIT_FOR_SIZE_OK"
                else:
                    state = "RUN_FFC"
            else:
            ...
            thRect_x, thRect_y, thRect_w, thRect_h = cv2.boundingRect(ff.getThEyesContours())
            ...
            writeLog(True)
            alreadyLogged = True
            state = "WAIT_FOR_NO_Eyes"


        elif state == "WAIT_FOR_NO_Eyes":
            if ff.getTcEyesContours(image) == True:
                state = "WAIT_FOR_NO_Eyes"
